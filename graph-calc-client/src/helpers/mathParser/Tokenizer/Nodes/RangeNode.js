...
RangeNode.prototype.needsEnd = function() {
  ...
  const endSymbols = this.filter( node => type.isSymbolNode(node) && (node.name === 'end') )
 ...
 * @private
 */
const compileRangeNode = (node, defs, args) => {
...
RangeNode.prototype.clone = function() {
  return new RangeNode(this.start, this.end, this.step)
}
 ...
 * @private
 */
const calculateNecessaryParentheses = (node, parenthesis) => {
  ...
  parens.start = ((startPrecedence !== null) &&
                 (startPrecedence <= precedence)) ||
                 (parenthesis === 'all')

  if(node.step) {
    const stepPrecedence = operators.getPrecedence(node.step, parenthesis)

    parens.step = ((stepPrecedence !== null) &&
                  (stepPrecedence <= precedence)) ||
                  (parenthesis === 'all')
  }

  const endPrecedence = operators.getPrecedence(node.end, parenthesis)

  parens.end = ((endPrecedence !== null) &&
               (endPrecedence <= precedence)) ||
               (parenthesis === 'all')
